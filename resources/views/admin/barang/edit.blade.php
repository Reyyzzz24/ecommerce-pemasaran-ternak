            ...
            <nav class="flex text-gray-700 bg-gray-50 py-3 px-5 rounded-lg border border-gray-200" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            <i class="fa fa-chevron-right text-gray-400 text-xs mx-2"></i>
                            <span class="text-gray-400 font-medium">Edit Barang</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="w-full px-2">
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100 bg-gray-50/50">
                    <h2 class="text-2xl font-bold text-gray-800">Edit Barang: {{ $barang->nama_barang }}</h2>
                </div>
                ...
                <div class="p-8">
                    ...
                    <form id="editForm" action="{{ route('admin.barang.update', $barang->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                <label for="gambar" class="text-sm font-semibold text-gray-700 flex items-center">
                                    <i class="fa fa-image mr-2 text-blue-500"></i> Update Gambar
                                </label>
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    <div class="flex-1">
                                        <input type="file" id="gambar" name="gambar" accept="image/*"
                                            class="w-full px-3 py-1 border border-gray-300 rounded-lg bg-white file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer">
                                        <p class="mt-1 text-xs text-gray-500 italic">Kosongkan jika tidak ingin mengubah gambar.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea id="keterangan" name="keterangan" rows="4" required
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition duration-200">{{ $barang->keterangan }}</textarea>
                        </div>

                        <div class="pt-4 flex gap-3">
                            <button type="submit" id="submitBtn" 
                                class="inline-flex items-center px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg shadow-lg hover:shadow-xl transition duration-300">
                                <i class="fa fa-save mr-2"></i> Update Barang
                            </button>
                            <a href="{{ route('admin.barang.index') }}" class="inline-flex items-center px-8 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-lg transition duration-300">
                                Batal
                            </a>
                        </div>
                    </form>
...
document.getElementById('editForm').addEventListener('submit', function(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            submitBtn.innerHTML = '<i class="fa fa-spinner fa-spin mr-2"></i> Mengupdate...';
